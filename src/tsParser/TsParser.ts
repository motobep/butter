...
import { TSPrograms } from './TsPrograms.ts'
...
const PAT_ID = 0x0;
const METADATA_ID_RANGE = [0x10, 0x1f];
const PID_MPEG1 = 0x03;
const PID_AAC = 0x0f;
const PID_ID3 = 0x15;
const PID_AVC = 0x1b;
const PID_HEVC = 0x24;
 ...
 * @constructor
 */
export const TSParser = function constructor() {
    this.size = 0;
    this.packets = [];
    this.pat;
    this.pmt;
...
TSParser.prototype._parsePackets = function _parsePackets(chunk) {
    ...
    while (pos <= (len - PACKET_SIZE)) {
        ...
        if (chunk[pos] === SYNC_BYTE) {
            ...
            if (packet.atf > 1) {
                let af = getAdaptationField()
                packet.af = af

                af.atflen = chunk[pos + 4];
                af.disc = !!(chunk[pos + 5] & 0x80);
                af.pcr = !!(chunk[pos + 5] & 0x10);

                if (af.pcr) {
                    // TODO: Obtain the Program clock reference (stored as 33 bits base, 6 bits reserved, 9 bits extension)
                    af.pcrPayload = new Uint8Array(6) // 48 bits
                    af.pcrPayload.set(chunk.subarray(pos + 6, pos + 6 + 6));

                    af.pcrBase = (af.pcrPayload[0] & 0xFF) * 33554432 +// 1 << 25 (33-8)
                        (af.pcrPayload[1] & 0xFF) * 131072 +// 1 << 17 (25-8)
                        (af.pcrPayload[2] & 0xFF) * 512 +// 1 << 9 (17-8)
                        (af.pcrPayload[3] & 0xFF) * 2 +// 1 << 1 (9-8)
                        (af.pcrPayload[4] & 0x80) / 128;
                    // check if greater than 2^32 -1
                    if (af.pcrBase > 4294967295) {
                        // decrement 2^33
                        af.pcrBase -= 8589934592;
                    }
                    af.pcrExt = (af.pcrPayload[4] & 0x01) * 256 + // 1 << 8
                        (af.pcrPayload[5] & 0xFF);
                    af.pcrValue = af.pcrBase * 300 + af.pcrExt;
                }

                offset = pos + 5 + af.atflen;
            ...
            if (this.pat && this.pat.pmtId && packet.pid === this.pat.pmtId) {
                ...
                } else if (packet.pid === this.pmt.mpeg1) {
                    stream = this.media.mpeg1;
                ...
                if (stream) {
                    ...
                    if (packet.pusi) {
                        if (stream.size > 0) {
                            let peshdr = doPes(stream)
                            if (peshdr) {
                                packet.pts = peshdr.pts
                                // console.log(peshdr)
                                // console.log(packet)
                            }
                        }
                ...
                switch (packet.pid) {
                    case PAT_ID:
                        ...
                        // console.log(`PMT:${this.pat.pmtId}`)
                        packet._type = 'PAT'
                        break;
                }
                if (METADATA_ID_RANGE[0] <= packet.pid 
                    && packet.pid <= METADATA_ID_RANGE[1]) {
                    packet._type = 'METADATA'
                }
            }
        } else {
...
function doPes(stream) {
    let pes = _parsePES(stream);
    let ret = null
    if (pes) {
        const peshdr = {};
        if (pes.pts) {
