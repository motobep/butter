...
export const TSPrograms = function constructor(pmt, packets, streams) {
  this.pmt = pmt;
...
TSPrograms.prototype.getTypes = function getTypes() {
  ...
  Object.keys(this.pmt).filter(programType => this.pmt[programType] !== -1).forEach((programType) => {
    ...
    program.pts = this.streams[program.type].peshdr.map(hdr => hdr.pts);
    program.dts = this.streams[program.type].peshdr.map(hdr => hdr.dts);
